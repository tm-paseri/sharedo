...
従来のチャットのみでやり取りをするところから発展しディレクトリの複数のファイルを読み込んでコーディングを行う, 差分を理解して修正するとより発展したAIのアシスタントを体験することができました

簡単なモックを作るだけであれば仕様書を書いてRooCodeに指示するだけでアプリが完成してしまいます。

とはいえ今回は技術的な指示を全く行っておらず実用的なアプリケーションの作成とは言えません
