...
とはいえVSCodeでの開発に慣れてしまい、新しいコードエディタには移行できずGitHub Copilotでコード補完を行うくらいの用途に留まってしまっていました。
...
今回はざっくりAIコーディングを体験することが目標のため、GitHub Copilotの無料プランを利用しています。
...
提示された仕様書はsystem_architectureやsecurity_requirementsなど運用を見据えた完全な仕様書のフォーマットのため、最低限のアプリの開発を行うには不要なchapterも多いです。

そこでRooCodeに作ってもらったものはサンプルとして残しつつ、本文はすべてTODOに直してもらい、後で肉付けできるようにしていきます。

まずなんでも良いのでファイルを1つ以下のように修正します。
...
完成したMarkdownがこちらです。同等の内容でファイルの修正を行ってくれています。
```
---
sidebar_position: 5
---
...
従来のチャットのみでやり取りをするところから発展し、ディレクトリの複数のファイルを読み込んでコーディングを行う、差分を理解して修正するといった、より発展したAIのアシスタントを体験することができました。

簡単なモックを作るだけであれば、仕様書を書いてRooCodeに指示するだけでアプリが完成してしまいます。

とはいえ今回は技術的な指示を全く行っておらず、実用的なアプリケーションの作成とは言えません。

次は以下の内容を含めてアプリを作ってもらおうと思います。

- 言語の指示
- ディレクトリの構成
- iOS/Android/デスクトップアプリなど稼働する媒体の指示